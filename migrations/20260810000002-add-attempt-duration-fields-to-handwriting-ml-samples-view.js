...
// Duration-correction pass (Part 12): extends handwriting_ml_samples with
// the tAbs-derived, ML-safe attempt_duration_ms/attempt_avg_speed/
// attempt_pause_frequency/attempt_pause_duration_ratio fields introduced in
// src/utils/featureNormalization.js's deriveTrajectoryFeatures() — same
// mechanism as migrations/20260810000001-add-speed-pause-fields-to-
// handwriting-ml-samples-view.js: extracted from the same
// `normalized_features` JSONB column every other derived feature already
// comes from.
//
// Straight CREATE OR REPLACE VIEW: every existing column (including the 6
// added by the previous migration) kept byte-for-byte identical, in its
// original order; the 4 new columns are appended at the end of each
// branch's SELECT list only. GET /handwriting/ml-samples/export selects
// `*` and is unaffected by appended columns.
//
// down() restores the exact prior (v2) view definition.
const CREATE_VIEW_SQL_V3 = `
...
SELECT
  ...
  (sf.normalized_features ->> 'attempt_duration_ms')::float         AS attempt_duration_ms,
  (sf.normalized_features ->> 'attempt_avg_speed')::float           AS attempt_avg_speed,
  (sf.normalized_features ->> 'attempt_pause_frequency')::float     AS attempt_pause_frequency,
  (sf.normalized_features ->> 'attempt_pause_duration_ratio')::float AS attempt_pause_duration_ratio
...
SELECT
  ...
  (la.normalized_features ->> 'attempt_duration_ms')::float         AS attempt_duration_ms,
  (la.normalized_features ->> 'attempt_avg_speed')::float           AS attempt_avg_speed,
  (la.normalized_features ->> 'attempt_pause_frequency')::float     AS attempt_pause_frequency,
  (la.normalized_features ->> 'attempt_pause_duration_ratio')::float AS attempt_pause_duration_ratio
FROM letter_attempts la
LEFT JOIN teacher_validation tv
  ON tv.student_id = la.student_id AND tv.collection_session_id = la.collection_session_id;
`;

// Exact prior (v2) definition — migrations/20260810000001-add-speed-pause-
// fields-to-handwriting-ml-samples-view.js — restored verbatim on rollback.
...
module.exports = {
  async up(queryInterface) {
    await queryInterface.sequelize.query(CREATE_VIEW_SQL_V3);
  },
